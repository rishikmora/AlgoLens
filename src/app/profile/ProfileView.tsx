...
/** Turns a display name into the handle used in the shareable URL. */
export const toHandle = (name: string) =>
  name.trim().toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") || "me";
...
export default function ProfileView({ handle }: { handle?: string }) {
  ...
  const slug = handle ?? toHandle(st.name);
  const shareUrl = mounted ? `${window.location.origin}/profile/${slug}` : `/profile/${slug}`;
  ...
  if (!mounted) return <div className="mx-auto max-w-5xl px-4 py-6 text-sm text-tertiary">Loading…</div>;
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex flex-col items-end gap-2">
            <Button size="sm" onClick={share}>
              {copied ? <Check className="text-signal" /> : <Share2 />}
              {copied ? "Link copied" : "Share profile"}
            </Button>
            <code className="rounded-xs border border-edge bg-elevated px-1.5 py-0.5 font-mono text-2xs text-tertiary">
              /profile/{slug}
            </code>
            <span className="flex items-center gap-1 text-2xs text-faint">
              <Globe className="size-3" /> public recruiter view
            </span>
          </div>
        </div>
      </Card>
